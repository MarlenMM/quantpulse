...
| `src/quantpulse/storage/` | `models.py` (SQLAlchemy ORM, 23 tables), `migrations/` (Alembic, 12 revisions), `persistence.py` (point-in-time read/write helpers — the only place outside a migration that touches a `Session` directly for most callers), `db.py` (engine/session factory) | `config.py` |
...
| `src/quantpulse/api/` | `main.py` (FastAPI app, 14 endpoints, all GET — asserted by a test over the OpenAPI schema), `schemas.py` (Pydantic response models) | `storage/persistence.py` — the *same* readers `app/` uses, so the two front ends can't disagree |
| `src/quantpulse/glossary.py`, `config.py` | UI-agnostic shared content (71 term definitions) and settings (pydantic-settings, reads `.env`) | — |
| `app/` | The Streamlit app: `Home.py` + `pages/1_Screener.py` … `6_Glossary.py`, `lib/` (thin `@st.cache_data` wrappers over `storage/persistence.py`, plus `refresh.py`, which owns the subprocess behind the Settings page's refresh button, and `charts.py`/`format.py`/`search.py`/`glossary.py` display helpers — `lib/` never contains scoring logic, only presentation) | `storage/persistence.py`, `quantpulse.analysis.*`, `quantpulse.llm.*` |
...
| `scripts/` | `seed_initial_data.py` (one-time, survivorship-bias-aware historical backfill) and `refresh_data.py` (the incremental refresh job, started from the app's Settings page or by dispatching the workflow — concurrent fetch phase via `ThreadPoolExecutor`, then a serial write phase — see the module's own docstring) | Everything above |
