...
| `src/quantpulse/api/` | `main.py` (FastAPI app, 11 endpoints, all GET — asserted by a test over the OpenAPI schema), `schemas.py` (Pydantic response models) | `storage/persistence.py` — the *same* readers `app/` uses, so the two front ends can't disagree |
