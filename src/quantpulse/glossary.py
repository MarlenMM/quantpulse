"""Plain-English definitions for every term the UIs show (Sections 10, 12).
...
**One definition, every surface.** This module is the single source of truth:
the Streamlit Glossary page renders `TERMS` in full, every `help=` tooltip
pulls from the same dict via `tip()`, and the REST API serves it to the React
frontend at `/api/glossary`. Writing a definition inline on a widget would
guarantee that the tooltip and the glossary drift apart the first time one of
them is edited — and a glossary that contradicts the tooltip beside it is worse
than no glossary.

It lives in `src/quantpulse/` rather than in `app/` because it is UI-agnostic
reference content, not presentation logic, and there are now two front ends
that need it. `app/lib/glossary.py` re-exports it so the Streamlit pages did
not have to change.
